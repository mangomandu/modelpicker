...
smaller model would nail. **fableite** puts a cheap triage step in front: a router
...
pytest                # 43 tests — the router model is mocked, so no API key is needed
```
...
`fableite route` call needs `ANTHROPIC_API_KEY` (or an `ant auth login` profile).
...
├─ llm.py       the mockable router-model judgment via the Anthropic SDK
├─ report.py    MetricsReport builder
└─ cli.py       `fableite route …`
tests/
