...
**사인(死因).** 실제 Claude Code 워크플로우에서 쓸모 있게 하려고, `UserPromptSubmit` 훅
(`hooks/escalation_nudge.py`)을 붙여 **모든** 의미 있는 프롬프트마다 `modelpicker route`를 호출해
알맞은 모델/effort를 추천하게 했습니다. 그런데 `route`는 — 기본값 `judge_backend: claude_cli`에서 —
풀 `claude -p` 판단 호출을 한 번 합니다. 즉, 토큰을 *안 쓰려고* 만든 도구가 **프롬프트 하나당 Claude
호출을 한 번씩 더** 일으킨 거예요. 프로젝트의 모든 세션에서, 조용히, 자동으로. 토큰이 줄긴커녕
치솟았습니다. 지출 *앞에* 서 있어야 할 라우터가 그 자체로 지출이 돼버린 거죠.

**진짜로 망가진 이유:**
- **판단은 공짜가 아니다.** `route` 한 번 = 모델 호출 한 번(~3초 `claude -p`). 어차피 승격할 무거운
  작업에 한 번 쓰는 건 괜찮지만, *매 턴* 자동으로 쏘면 파멸적입니다.
- **프롬프트당 자동화는 호출당 비용을 증폭한다.** `TRIVIAL_SKIP` 프리필터는 인사·오타수정 정도만
  걸러냈고, 의미 있는 작업은 전부 매번 "판단 세금"을 냈습니다.
- **한꺼번에 모든 곳에서 돌았다.** 훅이 프로젝트 범위라 *모든* 세션이 비용을 냈고, 어디에 귀속할지도
  없이 비용이 누적됐어요 — *"다른 세션에서도 이상해지는데."*
- **절약 계산이 엉뚱한 걸 쟀다.** 대문짝 "~46% 절약"은 라우팅 *결정*만 셌지 *결정하는 비용*은 한 번도
  안 셌습니다. 훅의 판단 호출까지 넣으면 절약은 뒤집혔어요 — 덜 쓰라는 말을 듣는 데 더 썼습니다.

**조치 / 최종 상태.** `.claude/settings.local.json`에서 훅 제거 → 프롬프트당 판단 호출 중단. repo
아카이브. 남길 교훈: **자동으로 돌리는 라우터는 그것이 아끼는 결정보다 싸야 한다 — 매 프롬프트마다
