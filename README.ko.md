...
**부활시킬까? 이 용도엔 아니다.** 재귀를 고쳐도, MCP로 `route`를 직접 부르는 건 그냥 모델에게
인라인으로 *"이거 Sonnet으로 충분해?"* 라고 묻는 것과 의미 있는 차이가 없다 — 오히려 **더 비쌀 수**
있다. 인라인 답변은 세션의 캐시된 컨텍스트를 재사용해 출력 토큰 몇백 개면 끝나지만, `route`는 매번
콜드부트 `claude -p`라 시스템 프롬프트를 풀값으로 다시 낸다. 라우터가 값을 하는 건 *"채팅 속 사람이
아닌 무언가"* 가 라우팅을 필요로 할 때뿐이다: 결정론적·튜닝가능·감사가능한 정책, 실제 실행을 싼 모델에
위임하는 `run`, 또는 비대화형 호출자(스크립트·CI·배치). 채팅에서 자기 모델 직접 고르는 솔로
개발자에겐 오버엔지니어링 — 그냥 물어보면 된다. 아카이브 유지; 코드와 테스트 68개는 언젠가 "시스템이
라우팅하는" 맥락이 생기면 쓰라고 남겨둔다.
